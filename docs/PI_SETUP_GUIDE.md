...
wget https://raw.githubusercontent.com/your-repo/sony-projector-control/main/scripts/setup_pi.sh
chmod +x setup_pi.sh
sudo ./setup_pi.sh
```
...
├── macropad/                 # Macropad control scripts
├── tests/                    # Test scripts
...
python3 macropad/macropad_control.py --layout 4

# Run with 9-button layout
python3 macropad/macropad_control.py --layout 9
```
...
ExecStart=/opt/projector-control/venv/bin/python3 macropad/macropad_control.py
Restart=always
RestartSec=10
...
- **Application logs**: `/opt/projector-control/logs/projector_control.log`
- **Debug logs**: Run `python3 utils/debug_monitor.py`
...
1. **Test basic connectivity** with `tests/test_connection.py`
...
6. **Set up monitoring** with `utils/debug_monitor.py`
