...
**✅ Note:** Freeze now uses the correct PJLink FREZ command (`%2FREZ 1`/`%2FREZ 0`) which properly pauses video without causing a black screen.
...
Control projectors with physical buttons using macropad scripts:

```bash
# Start 9-button macropad control system
python3 macropad/macropad_control.py

# Start 4-button macropad control system (essential functions only)
python3 macropad/macropad_4button.py

# Or use the flexible layout option
python3 macropad/macropad_control.py --layout 4  # 4 buttons
python3 macropad/macropad_control.py --layout 9  # 9 buttons (default)
```
...
See `HARDWARE_SETUP.md` for detailed hardware instructions.
...
python3 utils/debug_monitor.py
```
...
│   ├── hid_macropad_control.py  # HID macropad control
│   └── usb_keypad_control_specific.py  # USB keypad control
├── utils/
...
│   └── rear_projector_examples.py  # Rear projector examples
